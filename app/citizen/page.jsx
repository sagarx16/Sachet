...
function OverviewModule({ setActiveModule, openSos }) {
  return (
    <div className="flex flex-col gap-6">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 via-indigo-700 to-blue-800 p-6 text-white shadow-xl">
            ...
            <div className="px-3 py-3 mb-1 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center">
                  <span className="material-symbols-outlined text-[18px] text-white" style={{ fontVariationSettings: "'FILL' 1" }}>person_pin_circle</span>
                </div>
                <div>
                  <div className="text-xs font-extrabold text-slate-900">Citizen Portal</div>
                  <div className="text-xs text-slate-400 font-semibold uppercase tracking-wider" style={{ fontSize: '9px' }}>IN-NP Disaster Grid</div>
                </div>
              </div>
            </div>
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider px-3 py-2" style={{ fontSize: '10px' }}>Modules</div>
            {modules.map((m) => {
              const isActive = active === m.id;
              return (
                <button key={m.id} onClick={() => setActive(m.id)} className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-left text-sm font-semibold transition-all cursor-pointer ${isActive ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}>
                  ...
                  {m.badge && !isActive && <span className="text-xs font-bold text-red-600 bg-red-100 px-1.5 py-0.5 rounded-full" style={{ fontSize: '9px' }}>{m.badge}</span>}
                </button>
              );
            })}
          </div>
          <div className="px-3 pb-3 pt-3 border-t border-slate-100">
            <button onClick={openSos} className="sos-glow w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gradient-to-r from-red-600 to-red-500 text-white font-bold text-sm shadow-md cursor-pointer hover:from-red-700 hover:to-red-600 transition-all">
